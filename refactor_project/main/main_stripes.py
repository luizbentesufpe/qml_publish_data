"""
main_stripes.py  (refatorado)
=============================
Ablation S0→S4 no dataset Stripes (handcrafted, mesmo estilo do
cross_circle.py). Mesmo padrão estrutural do main_cross_circle.py /
main_breast_cancer.py refatorados:

  - Scenarios importados de refactor_project.config.ablation.stripes
  - Worker por seed em run_one_stripes._run_one_seed_stripes
  - Paralelismo via joblib.Parallel + loky
  - Flag DEBUG para rodar com ConfigDebug (episódios mínimos)
  - Output em publication_out_stripes/  (normal)
              ou debug_out_stripes/      (DEBUG=True)

Dataset:
  Handcrafted "stripes" — grid 3x3 (9 features), listras horizontais
  (classe 0) vs. verticais (classe 1) com ruído gaussiano. Mesmo
  formato de refactor_project.data.cross_circle (círculo vs. cruz).

Uso:
    python main_stripes.py            # publicação
    python main_stripes.py --debug    # debug rápido
"""
...
from refactor_project.config.ablation.stripes import STRIPES_SCENARIOS
from refactor_project.data.stripes import FEATURE_NAMES
...
from refactor_project.main.runner.run_one_stripes import _run_one_seed_stripes
...
def main_stripes(DEBUG: bool = False) -> None:
    """
    Ablation S0→S4 no dataset Stripes.

    Protocolo idêntico ao main_cross_circle() / main_breast_cancer():
    ...
    """
    print(f"\n{'=' * 68}")
    print("  ABLATION — Stripes (handcrafted, grid 3x3, horizontal vs. vertical)")
    ...
    if DEBUG:
        cfg0 = ConfigDebug()
        root_out = Path("debug_out_stripes")
        SEEDS = [0]
    else:
        cfg0 = Config()
        root_out = Path("publication_out_stripes")
        SEEDS = [0, 1, 2, 3, 4]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Ablation study on Stripes dataset")
    parser.add_argument(
        "--debug", action="store_true", help="Run in DEBUG mode (minimal episodes, 1 seed)",
    )
    args = parser.parse_args()

    main_stripes(DEBUG=args.debug)