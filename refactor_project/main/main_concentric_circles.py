"""
main_concentric_circles.py  (refatorado)
=========================================
Ablation S0→S4 no dataset Círculos Concêntricos (sklearn.make_circles).
Mesmo padrão estrutural do main_breast_cancer.py / main_iris.py
refatorados:

  - Scenarios importados de refactor_project.config.ablation.concentric_circles
  - Worker por seed em run_one_concentric_circles._run_one_seed_concentric_circles
  - Paralelismo via joblib.Parallel + loky
  - Flag DEBUG para rodar com ConfigDebug (episódios mínimos)
  - Output em publication_out_concentric_circles/  (normal)
              ou debug_out_concentric_circles/      (DEBUG=True)

Dataset:
  sklearn.datasets.make_circles — 2 features, fronteira RADIAL
  (não-linear no espaço original), 400 amostras, ~50/50.
  https://scikit-learn.org/stable/modules/generated/sklearn.datasets.make_circles.html

Uso:
    python main_concentric_circles.py            # publicação
    python main_concentric_circles.py --debug    # debug rápido
"""
...
from refactor_project.config.ablation.concentric_circles import CONCENTRIC_CIRCLES_SCENARIOS
from refactor_project.data.concentric_circles import FEATURE_NAMES
...
from refactor_project.main.runner.run_one_concentric_circles import (
    _run_one_seed_concentric_circles,
)
...
from refactor_project.util.util import (
    Logger,
    _pearson,
    _plot_alpha_bar_concentric_circles,
    _spearman,
    dump_run_metadata,
)
...
def main_concentric_circles(DEBUG: bool = False) -> None:
    """
    Ablation S0→S4 no dataset Círculos Concêntricos.
    ...
    """
    print(f"\n{'=' * 68}")
    print("  ABLATION — Círculos Concêntricos (sklearn.make_circles, 2 features)")
    ...
    if DEBUG:
        cfg0 = ConfigDebug()
        root_out = Path("debug_out_concentric_circles")
        SEEDS = [0]
    else:
        cfg0 = Config()
        root_out = Path("publication_out_concentric_circles")
        SEEDS = [0, 1, 2, 3, 4]
    ...
    scenarios = CONCENTRIC_CIRCLES_SCENARIOS
    ...
    for sc in scenarios:
        ...
        raw_results = Parallel(
            ...
            delayed(_run_one_seed_concentric_circles)(
                seed=seed,
                nq=nq,
                sc_name=sc_name,
                sc_tag=sc_tag_,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Ablation study on Concentric Circles dataset"
    )
    parser.add_argument(
    ...
    main_concentric_circles(DEBUG=args.debug)