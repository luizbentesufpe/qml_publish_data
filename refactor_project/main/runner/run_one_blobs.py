...
from refactor_project.data.blobs import _feature_names, load_blobs_pool
...
def _run_one_seed_blobs(
    ...
    uma combinação (seed, nq) do dataset Blobs.

    Diferente dos demais runners: n_features e cluster_std NÃO são
    fixos — são lidos de cfg_base (setados pelos overrides do
    cenário via apply_overrides), permitindo que o mesmo runner sirva
    tanto os cenários S0-S4 (2D) quanto os cenários de alta
    dimensionalidade (bl_hd_d4, bl_hd_d8, bl_hd_d16, bl_hd_d32).
    """
    _torch.set_num_threads(1)
    DEVICE = "cpu"
    set_seeds(seed)

    n_features = int(getattr(cfg_base, "n_features", 2))
    cluster_std = float(getattr(cfg_base, "cluster_std", 1.0))
    FEATURE_NAMES: List[str] = _feature_names(n_features)
    ...
    tr_idx, ho_idx = split_holdout(
        X_full, Y_full, frac=float(cfg_base.holdout_frac), seed=seed,
    )
    ...
    in_dim = int(XtrS.shape[1])  # == n_features
    ...
    arch_mat, best_nq, best_proxy = run_arch_search_end2end(
        XtrS, YtrS, XvaS, YvaS,
        cfg=cfg_nq, logger=nq_logger, seed=seed, device=DEVICE,
    )
    arch_mat = sanitize_architecture(arch_mat, int(best_nq))

    # ── Nested CV em TRAIN_ALL ────────────────────────────────────────────
    nested = nested_cv_eval_fixed_arch(
        arch_mat, X_train_all, Y_train_all,
        cfg=cfg_nq, n_qubits=int(best_nq), logger=nq_logger, seed=seed, device=DEVICE,
    )

    # ── Final holdout ─────────────────────────────────────────────────────
    auc_ho, sens_ho, thr_ho = train_final_model_end2end(
        arch_mat, int(best_nq), X_train_all, Y_train_all, X_holdout, Y_holdout,
        cfg_nq, nq_logger, device=DEVICE, noise=False,
    )

    # ── Final holdout (com ruído) ──────────────────────────────────────────
    auc_with_noise, sens_with_noise, thr_with_noise = train_final_model_end2end(
        arch_mat, int(best_nq), X_train_all, Y_train_all, X_holdout, Y_holdout,
        cfg_nq, noise_logger, device=DEVICE, noise=True,
    )

    # ── Lê α/β do logger isolado por seed ──────────────────────────────────
    _alpha_final: Optional[List[float]] = None
    _beta_final: Optional[List[float]] = None
    try:
        _pt_path = nq_dir / "logs" / "enc_params_final.pt"
        if _pt_path.exists():
            _enc = torch.load(str(_pt_path), map_location="cpu", weights_only=False)
