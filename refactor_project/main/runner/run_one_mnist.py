...
from refactor_project.data.mnist import (
    DEFAULT_DIGIT_PAIR,
...
def _run_one_seed_mnist(
    ...
    uma combinação (seed, nq) do dataset MNIST binarizado (3 vs 8,
    reduzido via PCA).
    """
    ...
    n_components = int(getattr(cfg_base, "feature_bank_size", DEFAULT_PCA_COMPONENTS))
    digit_pair = tuple(getattr(cfg_base, "digit_pair", DEFAULT_DIGIT_PAIR))
    FEATURE_NAMES: List[str] = _feature_names(n_components)
    ...
    in_dim = int(XtrS.shape[1])  # == n_components
    ...
    for attr in [
        "feature_bank_size", "n_features", "in_dim", "d_in", "input_dim", "n_inputs",
    ]:
