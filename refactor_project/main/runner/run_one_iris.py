...
from refactor_project.data.iris import load_iris_pool
...
#: Nomes semânticos das 4 features — usados nos JSONs e gráficos
FEATURE_NAMES: List[str] = [
    "sepal_length", "sepal_width", "petal_length", "petal_width"
]


def _run_one_seed_iris(
    seed: int,
    nq: int,
    sc_name: str,
    sc_tag: str,
    ...
    uma combinação (seed, nq) do dataset Iris (binarizado:
    ...
    data_dir      : diretório onde o CSV do Iris está (ou será baixado)
    """
    ...
    in_dim = int(XtrS.shape[1])  # 4 para Iris

    # ── Config por nq ─────────────────────────────────────────────────────
    cfg_nq = make_cfg_for_qubits(cfg_base, int(nq), n_train=len(XtrS), in_dim=in_dim)

    # Iris: 4 features — desativa feature bank dinâmico
