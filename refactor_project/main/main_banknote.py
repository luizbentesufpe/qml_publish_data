"""
main_banknote.py  (refatorado)
==============================
Ablation S0→S4 no dataset Banknote Authentication (UCI).
Mesmo padrão estrutural do main_cross_circle.py refatorado:
  ...
  - Worker por seed em run_one_banknote._run_one_seed_banknote
  - Paralelismo via joblib.Parallel + loky
  - Flag DEBUG para rodar com ConfigDebug (episódios mínimos)
  - Output em publication_out_banknote/  (normal)
              ou debug_out_banknote/      (DEBUG=True)

Dataset:
  UCI — Banknote Authentication (4 wavelet features, 1372 amostras)
  https://archive.ics.uci.edu/ml/datasets/banknote+authentication

Uso:
    python main_banknote.py            # publicação
    python main_banknote.py --debug    # debug rápido
"""
...
from refactor_project.main.runner.run_one_banknote import (
    FEATURE_NAMES,
    _run_one_seed_banknote,
)
...
def main_banknote(DEBUG: bool = False, data_dir: str = "data") -> None:
    """
    Ablation S0→S4 no dataset Banknote Authentication.
    ...
    """
    print(f"\n{'=' * 68}")
    print("  ABLATION — Banknote Authentication (UCI, 4 wavelet features)")
    ...
    if DEBUG:
        cfg0 = ConfigDebug()
        root_out = Path("debug_out_banknote")
        SEEDS = [0]
    else:
        cfg0 = Config()
        root_out = Path("publication_out_banknote")
        SEEDS = [0, 1, 2, 3, 4]
    ...
    print("[INFO] Verificando dataset Banknote ...")
    X_chk, Y_chk = create_banknote_dataset(seed=0, data_dir=data_dir)
    ...
    for sc in scenarios:
        ...
        raw_results = Parallel(
            ...
            delayed(_run_one_seed_banknote)(
                seed=seed,
                nq=nq,
                sc_name=sc_name,
                sc_tag=sc_tag_,
