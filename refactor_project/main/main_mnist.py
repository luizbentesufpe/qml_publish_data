"""
main_mnist.py  (refatorado)
===========================
Ablation S0→S4 no dataset MNIST binarizado (dígitos 3 vs. 8, reduzido
via PCA). Mesmo padrão estrutural do main_breast_cancer.py /
main_iris.py refatorados:

  - Scenarios importados de refactor_project.config.ablation.mnist
  - Worker por seed em run_one_mnist._run_one_seed_mnist
  - Paralelismo via joblib.Parallel + loky
  - Flag DEBUG para rodar com ConfigDebug (episódios mínimos)
  - Output em publication_out_mnist/  (normal)
              ou debug_out_mnist/      (DEBUG=True)

Dataset:
  MNIST (openml mnist_784) binarizado no par 3 vs. 8 — dígitos
  visualmente mais parecidos, benchmark difícil clássico — reduzido de
  784 pixels para 8 componentes PCA (necessário para angle encoding em
  poucos qubits). Único dataset do framework com imagens reais.

Uso:
    python main_mnist.py            # publicação
    python main_mnist.py --debug    # debug rápido
"""
...
from refactor_project.config.ablation.mnist import MNIST_SCENARIOS
from refactor_project.data.mnist import DEFAULT_DIGIT_PAIR, create_mnist_dataset
...
from refactor_project.main.runner.run_one_mnist import _run_one_seed_mnist
...
def main_mnist(DEBUG: bool = False, data_dir: str = "data") -> None:
    """
    Ablation S0→S4 no dataset MNIST binarizado (3 vs 8).

    Protocolo idêntico ao main_breast_cancer() / main_iris():
    ...
    Nota: pré-download/pré-processamento do MNIST completo (fetch_openml
    + PCA) é feito serialmente aqui antes do Parallel para evitar
    downloads concorrentes entre workers no primeiro lançamento —
    mesmo motivo do pré-download serial do Higgs.
    """
    print(f"\n{'=' * 68}")
    print("  ABLATION — MNIST binarizado (3 vs 8, PCA)")
    ...
    if DEBUG:
        cfg0 = ConfigDebug()
        root_out = Path("debug_out_mnist")
        SEEDS = [0]
    else:
        cfg0 = Config()
        root_out = Path("publication_out_mnist")
        SEEDS = [0, 1, 2, 3, 4]
    ...
    # ── valida dataset antes de iniciar (serial, evita downloads
    #    concorrentes do MNIST completo entre workers) ─────────────────
    print("[INFO] Verificando dataset MNIST (3 vs 8, pré-download serial)...")
    X_chk, Y_chk = create_mnist_dataset(seed=0, data_dir=data_dir)
    n_pos = int((Y_chk.flatten() > 0.5).sum())
    n_neg = int(len(Y_chk) - n_pos)
    d0, d1 = DEFAULT_DIGIT_PAIR
    print(
        f"[INFO] OK: {len(X_chk)} amostras | "
        f"digit_{d1}={n_pos} digit_{d0}={n_neg} | "
        f"balance={n_pos / len(Y_chk):.2%} | "
        f"features(PCA)={X_chk.shape[1]}"
    )
    ...
    for sc in scenarios:
        ...
        dump_run_metadata(
            ...
            extra={
                ...
                "digit_pair": list(DEFAULT_DIGIT_PAIR),
        ...
        if not corr_csv.exists():
            with open(corr_csv, "w", newline="") as f:
                csv.writer(f).writerow(
                    [
                        "scenario_tag", "seed", "nq_requested", "best_nq",
                        "best_proxy_rl", "final_auc_nested",
                    ]
                )
        ...
        for res in raw_results:
            ...
            all_pareto.append(
                {"seed": s, "n_qubits": nq_res, "cost": res["cost"], "perf": res["perf"]}
            )
        ...
        for seed in SEEDS:
            ...
            try:
                with open(corr_csv, "a", newline="") as f:
                    ...
                    for nq_str, obj in rlqcv.items():
                        w.writerow(
                            [
                                str(sc_tag_), int(seed), int(nq_str), obj["best_nq"],
                                ("" if obj["best_proxy_rl"] is None else float(obj["best_proxy_rl"])),
            ...
            best: Optional[Dict[str, Any]] = max(
                rlqcv.values(), key=lambda x: x["perf"], default=None,
            )
            if best is not None:
        ...
        try:
            ...
            if len(rows) >= 2:
                ...
                logger.log_to_file(
                    ...
                    f"pearson={pr:.3f} spearman={sr:.3f} (rl_proxy vs final_auc)",
                )
                scenario_result["corr_rlproxy_vs_finalauc"] = {
                    "n": int(len(rows)), "pearson": float(pr), "spearman": float(sr),
                }
        except Exception as e:
        ...
        all_scenarios_results.append(
            {
                "scenario": {
                    "name": sc_name, "tag": sc_tag_, "overrides": sc_over,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Ablation study on MNIST binary (3 vs 8, PCA-reduced) dataset"
    )
    parser.add_argument(
        "--debug", action="store_true", help="Run in DEBUG mode (minimal episodes, 1 seed)",
    )
    parser.add_argument(
        "--data-dir", type=str, default="data",
        help="Diretório de cache do fetch_openml (default: data/)",
    )
    args = parser.parse_args()

    main_mnist(DEBUG=args.debug, data_dir=args.data_dir)