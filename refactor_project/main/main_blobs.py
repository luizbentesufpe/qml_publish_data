"""
main_blobs.py  (refatorado)
===========================
Ablation S0→S4 no dataset Blobs (sklearn), incluindo varredura de
escalabilidade em alta dimensionalidade (2, 4, 8, 16, 32 features).
Mesmo padrão estrutural do main_breast_cancer.py / main_iris.py
refatorados:

  - Scenarios importados de refactor_project.config.ablation.blobs
  - Worker por seed em run_one_blobs._run_one_seed_blobs
  - Paralelismo via joblib.Parallel + loky
  - Flag DEBUG para rodar com ConfigDebug (episódios mínimos)
  - Output em publication_out_blobs/  (normal)
              ou debug_out_blobs/      (DEBUG=True)

Dataset:
  sklearn.datasets.make_blobs — classes linearmente separáveis por
  construção. Caso mais "fácil" do framework — controle de sanidade.

  Por padrão roda ALL_BLOBS_SCENARIOS (S0-S4 em 2D + bl_hd_d{4,8,16,32}
  usando a config "full pipeline" do S4). Use --scenarios para
  restringir.

Uso:
    python main_blobs.py                          # tudo (S0-S4 + high-dim)
    python main_blobs.py --debug                   # debug rápido
    python main_blobs.py --scenarios bl_s0_baseline bl_s4_focal
    python main_blobs.py --scenarios bl_hd_d4 bl_hd_d8 bl_hd_d16 bl_hd_d32
"""
...
from refactor_project.config.ablation.blobs import ALL_BLOBS_SCENARIOS
...
from refactor_project.main.runner.run_one_blobs import _run_one_seed_blobs
...
def main_blobs(
    ...
    """
    Ablation S0→S4 (+ varredura de alta dimensionalidade) no dataset Blobs.
    ...
    scenario_filter : se fornecido, roda só os cenários cujo "name"
                       está nessa lista (ex.: rodar só os de alta
                       dimensionalidade separadamente).
    """
    print(f"\n{'=' * 68}")
    print("  ABLATION — Blobs (sklearn, controle linearmente separável)")
    ...
    if DEBUG:
        cfg0 = ConfigDebug()
        root_out = Path("debug_out_blobs")
        SEEDS = [0]
    else:
        cfg0 = Config()
        root_out = Path("publication_out_blobs")
        SEEDS = [0, 1, 2, 3, 4]
    ...
    for sc in scenarios:
        ...
        n_features = int(sc_over.get("n_features", 2))

        print(f"\n{'─' * 68}")
        print(f"  Cenário: {sc_name}  [{sc_tag_}]  (n_features={n_features})")
        print(f"{'─' * 68}")
        ...
        try:
            ...
            if alpha_per_seed:
                ...
                _plot_alpha_bar_blobs(
                    ...
                    title=(
                        f"α convergido — {sc_name}\n(média ± std  ·  {len(alpha_per_seed)} seeds  ·  d={n_features})"
                    ),
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Ablation study on Blobs dataset (incl. testes de alta dimensionalidade)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Roda só os cenários com esses nomes (default: todos, S0-S4 + high-dim)",
    )
    args = parser.parse_args()

    main_blobs(DEBUG=args.debug, scenario_filter=args.scenarios)