...
def main_higgs(
    ...
    """
    Ablation S0→S4 no dataset HIGGS Boson.
    ...
    Específico ao HIGGS:
      - Subset balanceado de `subset_size` amostras (default 10k)
        do dataset original de 11M (intratável para QAS com nq <= 6)
      - Pré-download do .csv.gz antes do Parallel para evitar race
        conditions entre workers no primeiro lançamento.
    """
    print(f"\n{'=' * 68}")
    ...
    # IMPORTANTE: chamada serial força o download (~280 MB) + decodificação
    # ANTES do Parallel, evitando race conditions entre workers no primeiro
    # lançamento. Subsequentes runs reusam o cache em data/HIGGS.csv.gz.
    ...
    for sc in scenarios:
        ...
        # ── Barplots α por cenário ────────────────────────────────────
        # Higgs (d=28) gera dois plots:
        #   1. Barplot por feature — mantém a função banknote (com 28 barras
        #      o gráfico fica denso mas é informativo).
        #   2. Barplot agregado low-level vs high-level — diagnóstico físico
        #      específico do Higgs (resultado de capa potencial para o paper).
        try:
            ...
            if alpha_per_seed:
                # Plot 1: por feature (reusa função banknote — funciona
                # para qualquer d, apenas fica denso com d=28).
                ...
                # Plot 2: agregado low-level vs high-level (diagnóstico físico)
        ...
        # ── Estatística específica Higgs: ratio HL/LL agregado ────────
        ...
        scenario_result: Dict[str, Any] = {
            ...
            "alpha_physics": hl_ll_stats,  # diagnóstico físico Higgs-específico
            "baselines": {},
            "pareto_front": pf,
            "runs": scenario_runs,
        }
