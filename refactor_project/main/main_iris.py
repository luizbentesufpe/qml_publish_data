"""
main_iris.py  (refatorado)
==========================
Ablation S0→S4 no dataset Iris (UCI), binarizado versicolor vs virginica.
Mesmo padrão estrutural do main_breast_cancer.py / main_banknote.py refatorados:

  - Scenarios importados de refactor_project.config.build_ablations
  - Worker por seed em run_one_iris._run_one_seed_iris
  - Paralelismo via joblib.Parallel + loky
  - Flag DEBUG para rodar com ConfigDebug (episódios mínimos)
  - Output em publication_out_iris/  (normal)
              ou debug_out_iris/      (DEBUG=True)

Dataset:
  UCI — Iris (4 features, 150 amostras; aqui usamos o subconjunto
  binarizado versicolor vs virginica, 100 amostras — par não
  linearmente separável, benchmark padrão em Quantum Machine Learning)
  https://archive.ics.uci.edu/ml/datasets/iris

Uso:
    python main_iris.py            # publicação
    python main_iris.py --debug    # debug rápido
"""
...
from refactor_project.config.ablation.iris import IRIS_SCENARIOS
from refactor_project.data.iris import create_iris_dataset
...
from refactor_project.main.runner.run_one_iris import (
    FEATURE_NAMES,
    _run_one_seed_iris,
)
...
def main_iris(DEBUG: bool = False, data_dir: str = "data") -> None:
    """
    Ablation S0→S4 no dataset Iris (versicolor vs virginica).

    Protocolo idêntico ao main_breast_cancer() / main_banknote():
    ...
    """
    print(f"\n{'=' * 68}")
    print("  ABLATION — Iris (UCI, 4 features, versicolor vs virginica)")
    ...
    if DEBUG:
        cfg0 = ConfigDebug()
        root_out = Path("debug_out_iris")
        SEEDS = [0]
    else:
        cfg0 = Config()
        root_out = Path("publication_out_iris")
        SEEDS = [0, 1, 2, 3, 4]
    ...
    print("[INFO] Verificando dataset Iris (versicolor vs virginica) ...")
    X_chk, Y_chk = create_iris_dataset(seed=0, data_dir=data_dir)
    n_pos = int((Y_chk.flatten() > 0.5).sum())
    n_neg = int(len(Y_chk) - n_pos)
    print(
        f"[INFO] OK: {len(X_chk)} amostras | "
        f"virginica={n_pos} versicolor={n_neg} | "
        f"balance={n_pos / len(Y_chk):.2%} | "
        f"features={X_chk.shape[1]}"
    )

    scenarios = IRIS_SCENARIOS
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Ablation study on Iris dataset (versicolor vs virginica)"
    )
    parser.add_argument(
        "--debug",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default="data",
        help="Directory containing iris.data (default: data/)",
    )
    args = parser.parse_args()

    main_iris(DEBUG=args.debug, data_dir=args.data_dir)